shinyServer(function(input,output){
  ...
  output$line <- renderPlot({   
    ...
    # time series
    plot( t.referrals.sub, xlab = "Months", ylab = "Number of referrals", type="b", 
          pch=19, xaxt="n", xlim=c(0,13), ylim=c(0,max(t.referrals.sub)+3), bty="n", 
          main="Trend of monthly referrals per agencies from CNY Foundation", col="green" )
